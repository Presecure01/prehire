  ...
  const handlePay = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        'http://localhost:5001/api/recruiter/wallet-balance',
    ...
    <div style={styles.container}>
      ...
      <main style={{
        ...
        <div style={{ textAlign: 'center', marginTop: 8 }}>
          ...
          <div style={{ color: '#6B7280', marginTop: 8 }}>Funds are used for unlocking candidate details & contact them</div>
        </div>
          ...
          <div style={styles.cardBox}>
            ...
            {!showNewCard && (
              <>
                ...
                <button style={styles.addCardRow} onClick={() => (window.location.href = '/recruiter/add-card')}>
            ...
            <div style={styles.actionsRow}>
              ...
              <button onClick={() => (window.location.href = '/recruiter/add-card')} style={styles.secondaryBtn}>Add new card</button>
            </div>
          </div>
        </div>
...
const styles = {
  ...
  avatar: { width: 36, height: 36, borderRadius: '50%', overflow: 'hidden', background: '#E5E7EB' },
  avatarImg: { width: '100%', height: '100%', objectFit: 'cover' },
  avatarFallback: { width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#374151', fontWeight: 600 },
  amountRow: { display: 'flex', gap: 12, justifyContent: 'center', marginTop: 24, flexWrap: 'wrap' },
  amountBtn: { padding: '10px 20px', borderRadius: 10, border: '1px solid #D1D5DB', background: '#fff', cursor: 'pointer' },
  amountBtnActive: { background: '#2563EB', color: '#fff', borderColor: '#2563EB' },
  customInput: { width: 140, padding: '10px 12px', border: '1px solid #D1D5DB', borderRadius: 10 },
  radioLabel: { display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' },
  cardBox: { marginTop: 16, background: '#F9FAFB', border: '1px solid #E5E7EB', borderRadius: 12, padding: 16 },
  savedCard: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', border: '1px solid #E5E7EB', borderRadius: 10, padding: '12px 16px', marginBottom: 10, background: '#fff' },
  subtle: { color: '#6B7280', fontSize: 12 },
  addCardRow: { width: '100%', textAlign: 'left', border: '1px solid #E5E7EB', borderRadius: 10, padding: '12px 16px', background: '#fff', cursor: 'pointer' },
  formGrid: { display: 'grid', gap: 12 },
  fieldCol: { display: 'flex', flexDirection: 'column' },
  fieldRow: { display: 'flex', gap: 12 },
  label: { fontSize: 12, color: '#6B7280', marginBottom: 6 },
  input: { padding: '10px 12px', border: '1px solid #D1D5DB', borderRadius: 10, width: '100%' },
  actionsRow: { display: 'flex', gap: 12, marginTop: 12 },
  payBtn: { background: '#2563EB', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 16px', cursor: 'pointer', fontWeight: 600 },
  secondaryBtn: { background: '#fff', color: '#111827', border: '1px solid #9CA3AF', borderRadius: 8, padding: '10px 16px', cursor: 'pointer', fontWeight: 600 },
  accountCard: { marginTop: 24, background: 'linear-gradient(135deg, #EEF2FF 0%, #F5F3FF 100%)', border: '1px solid #E5E7EB', borderRadius: 12, padding: 16 }
};
